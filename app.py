...
st.title("Comparativo de Rede: Múltiplas Unidades")
st.markdown("Visão consolidada de consumo entre diferentes unidades de saúde/tempo.")
# Função para criar dados aleatórios de demonstração
def gerar_dados_demo():
    ...
    for unidade in unidades:
        # Gera matriz aleatória (Medicamentos x Dias)
        ...
        df_long['Unidade'] = unidade # Identifica a origem
        lista_dfs.append(df_long)
    return pd.concat(lista_dfs, ignore_index=True)
# Upload de Arquivos (Múltiplos na barra lateral)
with st.sidebar:
    st.header("Carregar Planilhas")
    st.markdown("Selecione **um arquivo por unidade**. O nome do arquivo será o nome da unidade.")
    arquivos = st.file_uploader(
        "Upload Excel/CSV (Layout: Med x Dias)", 
        type=['csv', 'xlsx'], 
    ...
    if arquivos:
        ...
        for arquivo in arquivos:
            try:
                ...
                col_med = df_temp.columns[0] # Assume 1ª coluna como nome do medicamento
                cols_dias = df_temp.columns[1:] # Restante são dias
...
df_master["Quantidade"] = pd.to_numeric(df_master["Quantidade"], errors='coerce')
...
with col_g1:
    ...
    # Prepara dados para o gráfico nativo do Streamlit
